<script lang="ts">
        ...
        // Available roles for new users.  Include group_owner so it can be
        // selected in the UI when creating a user.
                        ...
                        if (inputFiles) {
                                ...
                                reader.onload = async (e) => {
                                        ...
                                        for (const [idx, row] of rows.entries()) {
                                                ...
                                                if (idx > 0) {
                                                        // ensure the role provided in the CSV matches one of the valid roles
                                                        if (columns.length === 4 && ROLE_OPTIONS.includes(columns[3].toLowerCase())) {
                                                                const res = await addUser(
                                                                        localStorage.token,
                                                                        columns[0],
                                                                        columns[1],
                                        ...
                                        <div class="px-1">
                                                {#if tab === ''}
                                                        <div class="flex flex-col w-full mb-3">
                                                                ...
                                                                <div class="flex-1">
                                                                        <!-- Use a single select element bound to _user.role.  Generate options
                                                                        dynamically based on ROLE_OPTIONS. -->
                                                                        <select
